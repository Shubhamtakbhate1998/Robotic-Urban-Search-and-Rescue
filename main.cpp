...
struct follower_pose {
  double x = 0;                                                                                                                         // Member 'x' contains x-coordinate of follower target location in map frame.
  double y = 0;                                                                                                                         // Member 'y' contains y-coordinate of follower target location in map frame.
  unsigned short int fiducial_id = 0;                                                                                                   // Member 'fiducial_id' contains the fiducial_id of the explorer-detected aruco tag.
};

follower_pose follower_locations[5];                                                                                                    // Follower locations
...
short int current_explorer_target = -1;                                                                                                 // Member 'current_explorer_target' contains the target number to be visited by explorer.
short int current_explorer_fiducial_id = -1;                                                                                            // Member 'current_explorer_fiducial_id' contains the latest fiducial_id detected by explorer.                    
short int current_follower_target = -1;                                                                                                 // Member 'current_follower_target' contains the target number to be visited by follower.
}psi;
...
void fiducial_callback(const fiducial_msgs::FiducialTransformArray::ConstPtr &msg)
{
  ...
  { 
    ...
    anotherOne.child_frame_id = "better_marker_frame";
...
void listen(tf2_ros::Buffer &tfBuffer, bool& status)
{
  ...
  try
  {
    transformStamped = tfBuffer.lookupTransform("map", "better_marker_frame", ros::Time(0));
...
int main(int argc, char **argv)
{
  ...
  bool explorer_goal_sent = false;                                                                    // boolean to check if explorer goal is sent.
  bool follower_goal_sent = false;                                                                    // boolean to check if follower goal is sent.
  bool no_more_exploring = false;                                                                     // boolean to check if any more goals are remaining.
  bool job_done = false;                                                                              // boolean to check if the aruco marker frame is localized in map.
  bool start_looking = false;                                                                         // boolean to determine when to start looking for aruco marker.

  std::array<std::array<double, 2>, 5> explorer_targets{};                                            // Array holding target locations of explorer. 
  std::array<std::pair<double, double>, 5> follower_targets{};                                        // Array holding target locations of follower.
  ...
  ros::Publisher explorer_pub = nh.advertise<geometry_msgs::Twist>("explorer/cmd_vel", 5);            // Publisher to make the explorer rotate when it reaches a target location so as to start detection of the aruco marker.  
  ros::Subscriber sub = nh.subscribe("/fiducial_transforms", 5, &fiducial_callback);                  // Subscriber for retrieving information about aruco tag.
...
void get_explorer_targets(ros::NodeHandle &nh, std::array<std::array<double, 2>, 5>& arr){
  for (int i = 0; i < 4 ; i++){
    ...
    arr[i][0] = my_list[0];                                                                           // Add x-coordinate of target into array.
    arr[i][1] = my_list[1];                                                                           // Add y-coordinate of target into array.
  }
